...
export class LoginComponent implements OnInit {
  ...
  constructor(private service: CapstoneService, private toastService: ToastService, private router: Router) {
    this.loginForm = new FormGroup({
      loginId: new FormControl('', Validators.required),
      psw: new FormControl('', [Validators.required, Validators.pattern('^(?=.*[a-z])(?=.*[A-Z])(?=.*[0-9])(?=.*[$@$!%*?&])[A-Za-z\d$@$!%*?&].{7,}$')])
    });
  }
  ...
  doLogin() {
    ...
    this.service.login(loginRequest).subscribe((res: CapstoneResponse) => {
